...
public class OLD_ItemService {
    ...
    public Page<MainItemDto> getMainItemPage(Pageable pageable){
        List<OLD_Item> result = this.OLDItemRepository.findAll();
        List<MainItemDto> list = new ArrayList<>();
        for (OLD_Item OLDItem : result) {
            MainItemDto dto = new MainItemDto();
            dto.setId(OLDItem.getId());
            dto.setItemNm(OLDItem.getItemNm());
            dto.setItemDetail(OLDItem.getItemDetail());
            dto.setPrice(OLDItem.getPrice());
            list.add(dto);
        }
